...
const Product = ({ product, admin }) => {
  ...
  return (
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow = '0 8px 40px rgba(0, 153, 255, 0.5)';
      }}
      onMouseLeave={(e) => {
    ...
    <Link to={`/product/${product._id}`} style={{ textDecoration: 'none', color: 'inherit' }} >
      ...
      <Card.Body style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <Card.Title className='text-center' style={{ height: '80px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        { product.name }
        </Card.Title>
        <Card.Text className='text-center' style={{ height: '60px', overflow: 'hidden' }}>
          {product.isSale ? (
            <>
      ...
      {admin && (
        ...
        <DeleteOutlined style= {{ fontSize: '1.3rem', color: '#ff4d4f', cursor:'pointer', backgroundColor: '#f0f0f0', borderRadius: '50%', padding: '8px', boxShadow:'0 2px 8px rgba(0, 0, 0, 0.1)', transition:'all 0.3s ease', display:'inline-flex', alignItems:'center', justifyContent:'center', marginLeft: '45px' }}  onMouseEnter={(e) => {
  ...
  onMouseLeave={(e) => {
    ...
    e.currentTarget.style.transform = 'scale(1)';
  }} title="Delete Product" onClick={handleDelete} />
        </div>
      )}
